...
function findPackageConfig(location: string, tries = 10): PackageConfig {
  ...
  if (fs.existsSync(packagePath)) {
    // eslint-disable-next-line @typescript-eslint/no-var-requires, prefer-destructuring, import/no-dynamic-require, global-require, @typescript-eslint/no-shadow
  ...
  return {
    ...
    binaries: [
      ...
      return {
        ...
        arch,
        binary,
        binaryChecksum: `${binary}${CHECKSUM_SUFFIX}`,
      ...
      .then(() =>
        data.isWindows
          ? fs
              ...
              .on('entry', (entry: { autodrain: () => unknown }) =>
                entry.autodrain()
              )
              .promise()
          : tar.x({
...
export function getBinaryEntry(platform?: string, arch?: string): BinaryEntry {
  ...
  for (const value of CONFIG.binaries) {
    if (
